...
// Data functions (insert, update, delete, form) for table divpol2022

// This script and data application were generated by AppGini 22.14
// Download AppGini for free from https://bigprof.com/appgini/download/

function divpol2022_insert(&$error_message = '') {
	global $Translation;

	// mm: can member insert record?
	$arrPerm = getTablePermissions('divpol2022');
	...
	// hook: divpol2022_before_insert
	if(function_exists('divpol2022_before_insert')) {
		$args = [];
		if(!divpol2022_before_insert($data, getMemberInfo(), $args)) {
	...
	insert('divpol2022', backtick_keys_once($data), $error);
	if($error) {
		$error_message = $error;
		return false;
	}
	...
	update_calc_fields('divpol2022', $recID, calculated_fields()['divpol2022']);

	// hook: divpol2022_after_insert
	if(function_exists('divpol2022_after_insert')) {
		$res = sql("SELECT * FROM `divpol2022` WHERE `PUESTO`='" . makeSafe($recID, false) . "' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)) {
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = makeSafe($recID, false);
		$args = [];
		if(!divpol2022_after_insert($data, getMemberInfo(), $args)) { return $recID; }
	}

	// mm: save ownership data
	set_record_owner('divpol2022', $recID, getLoggedMemberID());

	// if this record is a copy of another record, copy children if applicable
	if(strlen(Request::val('SelectedID'))) divpol2022_copy_children($recID, Request::val('SelectedID'));

	return $recID;
}

function divpol2022_copy_children($destination_id, $source_id) {
	global $Translation;
...
function divpol2022_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	// insure referential integrity ...
	global $Translation;
	$selected_id = makeSafe($selected_id);

	// mm: can member delete record?
	if(!check_record_permission('divpol2022', $selected_id, 'delete')) {
		return $Translation['You don\'t have enough permissions to delete this record'];
	}

	// hook: divpol2022_before_delete
	if(function_exists('divpol2022_before_delete')) {
		$args = [];
		if(!divpol2022_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
	...
	// child table: lideres
	$res = sql("SELECT `PUESTO` FROM `divpol2022` WHERE `PUESTO`='{$selected_id}'", $eo);
	$PUESTO = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `lideres` WHERE `PUESTO`='" . makeSafe($PUESTO[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
		$RetMsg = str_replace('<TableName>', 'lideres', $RetMsg);
		return $RetMsg;
	} elseif($rirow[0] && $AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation['confirm delete'];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
		$RetMsg = str_replace('<TableName>', 'lideres', $RetMsg);
		$RetMsg = str_replace('<Delete>', '<input type="button" class="btn btn-danger" value="' . html_attr($Translation['yes']) . '" onClick="window.location = \'divpol2022_view.php?SelectedID=' . urlencode($selected_id) . '&delete_x=1&confirmed=1&csrf_token=' . urlencode(csrf_token(false, true)) . '\';">', $RetMsg);
		$RetMsg = str_replace('<Cancel>', '<input type="button" class="btn btn-success" value="' . html_attr($Translation[ 'no']) . '" onClick="window.location = \'divpol2022_view.php?SelectedID=' . urlencode($selected_id) . '\';">', $RetMsg);
		return $RetMsg;
	}

	sql("DELETE FROM `divpol2022` WHERE `PUESTO`='{$selected_id}'", $eo);

	// hook: divpol2022_after_delete
	if(function_exists('divpol2022_after_delete')) {
		$args = [];
		divpol2022_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("DELETE FROM `membership_userrecords` WHERE `tableName`='divpol2022' AND `pkValue`='{$selected_id}'", $eo);
}

function divpol2022_update(&$selected_id, &$error_message = '') {
	global $Translation;

	// mm: can member edit record?
	if(!check_record_permission('divpol2022', $selected_id, 'edit')) return false;
	...
	// get existing values
	$old_data = getRecord('divpol2022', $selected_id);
	...
	// hook: divpol2022_before_update
	if(function_exists('divpol2022_before_update')) {
		$args = ['old_data' => $old_data];
		if(!divpol2022_before_update($data, getMemberInfo(), $args)) {
			if(isset($args['error_message'])) $error_message = $args['error_message'];
			return false;
		}
	}
	...
	if(!update(
		...
		echo $error_message;
		echo '<a href="divpol2022_view.php?SelectedID=' . urlencode($selected_id) . "\">{$Translation['< back']}</a>";
		exit;
	}
	...
	update_calc_fields('divpol2022', $data['selectedID'], calculated_fields()['divpol2022']);

	// hook: divpol2022_after_update
	if(function_exists('divpol2022_after_update')) {
		$res = sql("SELECT * FROM `divpol2022` WHERE `PUESTO`='{$data['selectedID']}' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)) $data = array_map('makeSafe', $row);

		$data['selectedID'] = $data['PUESTO'];
		$args = ['old_data' => $old_data];
		if(!divpol2022_after_update($data, getMemberInfo(), $args)) return;
	}

	// mm: update ownership data
	sql("UPDATE `membership_userrecords` SET `dateUpdated`='" . time() . "', `pkValue`='{$data['PUESTO']}' WHERE `tableName`='divpol2022' AND `pkValue`='" . makeSafe($selected_id) . "'", $eo);

	// if PK value changed, update $selected_id
	$selected_id = $newID;
}

function divpol2022_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $separateDV = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	if($selected_id) {
		...
		// mm: who is the owner?
		$ownerGroupID = sqlValue("SELECT `groupID` FROM `membership_userrecords` WHERE `tableName`='divpol2022' AND `pkValue`='" . makeSafe($selected_id) . "'");
		$ownerMemberID = sqlValue("SELECT LCASE(`memberID`) FROM `membership_userrecords` WHERE `tableName`='divpol2022' AND `pkValue`='" . makeSafe($selected_id) . "'");
		...
		$res = sql("SELECT * FROM `divpol2022` WHERE `PUESTO`='" . makeSafe($selected_id) . "'", $eo);
		if(!($row = db_fetch_array($res))) {
			return error_message($Translation['No records found'], 'divpol2022_view.php', false);
		}
	...
	if($dvprint) {
		$template_file = is_file("./{$TemplateDVP}") ? "./{$TemplateDVP}" : './templates/divpol2022_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	} else {
		$template_file = is_file("./{$TemplateDV}") ? "./{$TemplateDV}" : './templates/divpol2022_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	// process buttons
	if($AllowInsert) {
		if(!$selected_id) $templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-success" id="insert" name="insert_x" value="1" onclick="return divpol2022_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save New'] . '</button>', $templateCode);
		$templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-default" id="insert" name="insert_x" value="1" onclick="return divpol2022_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save As Copy'] . '</button>', $templateCode);
	} else {
	...
	if($selected_id) {
		...
		if($AllowUpdate) {
			$templateCode = str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return divpol2022_validateData();" title="' . html_attr($Translation['Save Changes']) . '"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		} else {
	...
	$rdata = $jdata = get_defaults('divpol2022');
	if($selected_id) {
		$jdata = get_joined_record('divpol2022', $selected_id);
		if($jdata === false) $jdata = get_defaults('divpol2022');
		$rdata = $row;
	}
	$templateCode .= loadView('divpol2022-ajax-cache', ['rdata' => $rdata, 'jdata' => $jdata]);

	// hook: divpol2022_dv
	if(function_exists('divpol2022_dv')) {
		$args = [];
		divpol2022_dv(($selected_id ? $selected_id : FALSE), getMemberInfo(), $templateCode, $args);
	}
